#!/bin/bash
# Ablation of train_mtlfd_all_tasks_noflip.sh: color_jitter disabled (see
# experiments/pick_place_mtlfd_all_tasks_noflip_nojitter.yaml's note for the rationale). Same
...
#SBATCH --export=ALL
#SBATCH --job-name=osvi_mtlfd_nojitter
#SBATCH --output=mtlfd_adaptation/slurm_logs/train_nojitter_%j.out
...
EXPERIMENT_FILE="${EXPERIMENT_FILE:-mtlfd_adaptation/experiments/pick_place_mtlfd_all_tasks_noflip_nojitter.yaml}"
WORKERS=8
...
LAUNCHER_SCRIPT="mtlfd_adaptation/train_mtlfd_all_tasks_noflip_nojitter.sh"
OUTPUT_LOG="mtlfd_adaptation/slurm_logs/train_nojitter_${SLURM_JOB_ID}.out"
