"""
Auto-resubmitting wrapper for the "tasks 0,5,10,15 held out of training" adapted-baseline
comparison run. Duplicate of run_train_mtlfd.py, pointed at
pick_place_mtlfd_heldout_0_5_10_15.yaml/train_mtlfd_heldout_0_5_10_15.sh - see
run_train_mtlfd.py's own docstring for the resume-mechanics background (unchanged here).

Usage (from a login node / tmux session):
    python3 mtlfd_adaptation/run_train_mtlfd_heldout_0_5_10_15.py [--max-restarts N]
"""
import argparse
import glob
import os
...
TRAIN_SH = os.path.join(REPO_ROOT, "mtlfd_adaptation", "train_mtlfd_heldout_0_5_10_15.sh")
EXPERIMENT_YAML = os.path.join(REPO_ROOT, "mtlfd_adaptation", "experiments", "pick_place_mtlfd_heldout_0_5_10_15.yaml")
...
def wait_for_job(job_id, poll_interval=POLL_INTERVAL_S):
    print(f"[run_train_heldout] waiting for job {job_id} to finish...", flush=True)
    while True:
        result = subprocess.run(["squeue", "-j", job_id, "-h"], capture_output=True, text=True)
        if not result.stdout.strip():
            print(f"[run_train_heldout] job {job_id} is no longer in the queue.", flush=True)
            return
...
def main():
    ...
    print(f"[run_train_heldout] target steps: {max_steps}", flush=True)

    if args.attach_job_id is not None:
        wait_for_job(args.attach_job_id)
    else:
        job_id = submit(resume_from=None)
        print(f"[run_train_heldout] submitted fresh job {job_id}", flush=True)
        wait_for_job(job_id)

    for restart_i in range(1, args.max_restarts + 1):
        ckpt_dir = find_checkpoint_dir()
        if ckpt_dir is None:
            print("[run_train_heldout] no checkpoint directory found after the job ended - "
        ...
        if step is None:
            print(f"[run_train_heldout] {ckpt_dir} exists but has no model_save-*.pt yet - "
                  "the job died before the first save_freq interval. Not resuming blind. "
                  "Exiting.", flush=True)
            sys.exit(1)

        print(f"[run_train_heldout] highest checkpoint step in {ckpt_dir}: {step}", flush=True)
        if step >= max_steps:
            print(f"[run_train_heldout] reached target ({step} >= {max_steps}). Done.", flush=True)
            return

        print(f"[run_train_heldout] restart {restart_i}/{args.max_restarts}: resuming from "
              f"{ckpt_dir} (step {step})", flush=True)
        job_id = submit(resume_from=ckpt_dir)
        print(f"[run_train_heldout] submitted resume job {job_id}", flush=True)
        wait_for_job(job_id)

    print(f"[run_train_heldout] hit --max-restarts ({args.max_restarts}) without reaching "
