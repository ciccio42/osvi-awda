#!/bin/bash
# Ablation of train_mtlfd_all_tasks_noflip.sh: samples_per_task: 2 (exact per-batch task balance,
# see experiments/pick_place_mtlfd_all_tasks_noflip_balanced.yaml's note for the rationale). Same
...
#SBATCH --export=ALL
#SBATCH --job-name=osvi_mtlfd_balanced
#SBATCH --output=mtlfd_adaptation/slurm_logs/train_balanced_%j.out
...
EXPERIMENT_FILE="${EXPERIMENT_FILE:-mtlfd_adaptation/experiments/pick_place_mtlfd_all_tasks_noflip_balanced.yaml}"
WORKERS=8
...
LAUNCHER_SCRIPT="mtlfd_adaptation/train_mtlfd_all_tasks_noflip_balanced.sh"
OUTPUT_LOG="mtlfd_adaptation/slurm_logs/train_balanced_${SLURM_JOB_ID}.out"
