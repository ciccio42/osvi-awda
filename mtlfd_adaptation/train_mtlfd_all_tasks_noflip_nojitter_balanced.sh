#!/bin/bash
# Ablation of train_mtlfd_all_tasks_noflip.sh: BOTH color_jitter disabled AND samples_per_task: 2
# together (see experiments/pick_place_mtlfd_all_tasks_noflip_nojitter_balanced.yaml's note). Same
# auto-resume-on-timeout mechanism as the baseline launcher (_auto_resume_lib.sh).
...
#SBATCH --export=ALL
#SBATCH --job-name=osvi_mtlfd_nojitter_balanced
#SBATCH --output=mtlfd_adaptation/slurm_logs/train_nojitter_balanced_%j.out
...
EXPERIMENT_FILE="${EXPERIMENT_FILE:-mtlfd_adaptation/experiments/pick_place_mtlfd_all_tasks_noflip_nojitter_balanced.yaml}"
WORKERS=8
...
LAUNCHER_SCRIPT="mtlfd_adaptation/train_mtlfd_all_tasks_noflip_nojitter_balanced.sh"
OUTPUT_LOG="mtlfd_adaptation/slurm_logs/train_nojitter_balanced_${SLURM_JOB_ID}.out"
